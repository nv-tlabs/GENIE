...
def get_loss_fn(config):
    ...
    def loss_fn(diffusion_wrapper, genie_model, x, y=None):
        ...
        eps_pred, xemb, temb = diffusion_wrapper.eps(perturbed_data, t, y=y, context=(
            cond_signal, aug_level), return_embeddings=True)

        eval_point = eps_pred / \
            torch.sqrt(1. + gamma_t ** 2.) - perturbed_data * \
            gamma_t / (1. + gamma_t ** 2.)
        deps_dx_backprop = jvp(lambda x: diffusion_wrapper.eps(
            x, t, y=y, context=(cond_signal, aug_level)), perturbed_data, eval_point)[1]
        deps_dt_backprop = jvp(lambda t: diffusion_wrapper.eps(perturbed_data, t, y=y, context=(
            cond_signal, aug_level)), t, v=torch.ones_like(t, device=t.device))[1] * dt_dgamma_t
        deps_dgamma_backprop = deps_dt_backprop + deps_dx_backprop

        deps_dgamma = genie_model(
            perturbed_data, t, eps, xemb, temb, context=(cond_signal, aug_level))
...
def training(config, workdir, mode):
    ...
    if config.optim.decay_scheduler is not None:
        ...
        def lambda_fn(step): return 1. - float(step) / \
            config.optim.decay_scheduler
